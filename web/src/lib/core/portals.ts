 ...
 * it via JOBBER_PORTALS, so an ad-hoc search can't clobber the curated config.
...
export function writeTempPortals(f: FilterLists): string {
  const file = path.join(os.tmpdir(), `jobber-explore-${randomUUID()}.yml`);
...
export function cleanupTempPortals(file: string): void {
  try {
    if (file.startsWith(os.tmpdir()) && file.includes("jobber-explore-")) fs.unlinkSync(file);
  } catch {
    /* best-effort */
  }
}
...
function loadYaml(rel: string): Record<string, unknown> | null {
  try {
    const doc = yaml.load(fs.readFileSync(path.join(jobberRoot(), rel), "utf8"));
