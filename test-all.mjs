...
// Moved to tests/cli-docs-skill-integrity.test.mjs (auto-discovered):
// AGENTS.md required sections, CLAUDE.md/CODEX.md/OPENCODE.md/GEMINI.md
// wrapper contracts, Codex documentation coverage, the per-CLI skill
// symlink chain + materialization (#1051), and the VERSION semver check.

// Moved to tests/scan-archive-location-filter.test.mjs (auto-discovered):
// archive-posting.mjs dry-run detection + live-render smoke test, and
// scan.mjs's location_filter always_allow tier + keyword matching.

// Moved to tests/followup-tracker-lifecycle.test.mjs (auto-discovered):
// followup-cadence.mjs date helpers, contact extraction, cadence CLI +
// e2e run; tracker report-link normalization (#760); and the
// deterministic cold-start onboarding-state detector.

// Moved to tests/batch-*.test.mjs (auto-discovered, one subprocess spawn
// per file so no single file exceeds test-runner.mjs's per-file timeout
// under --parallel contention): scan.mjs's URL rediscovery fallback,
// batch-runner.sh's rate-limit pause + spend_tier model routing +
// --strict-mcp-config isolation (#506), and update-system.mjs's
// SEMVER_RE parsing (#923).
