...
**Status:** plan of record, 2026-09-08. Answers two product requirements for the hosted tier: **the user uploads a CV**, and **the user does the job by chatting with an agent**. Model routing is decided and applied in code (`lib/llm-providers.mjs` → `HOSTED_ROUTES`); everything else here is unbuilt.
...
| 1 | **Runner** | `agent-runner.mjs`: OpenRouter tool loop, Claude-shaped NDJSON out, tool set of §2.3, usage accounting; `probe-providers.mjs` (post-`deny` pools) | fallback test: mocked 429 on primary → second answers, *and* 200 on primary → primary answers; path-escape refused *and* legit read succeeds; a hostile-JD prompt produces no write |
| 2 | **Tenancy** | `provision-workspace.mjs` from the contract; `jobberRoot()` via `AsyncLocalStorage`; `KNOWN` gets `workler`; `/api/run` hosted branch → the two scripts | two workspaces cannot read each other's `cv.md` (positive read in own, refused read across); an evaluation through the web is byte-identical to the CLI's for the same fixture |
