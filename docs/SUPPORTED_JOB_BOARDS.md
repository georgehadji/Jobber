...
| APEC | API | France's national board for cadre/executive roles. Configure with `provider: apec` and an `apec:` block (`keywords`, falling back to `config/profile.yml` target_roles). POSTs to the public `rechercheOffre` webservice apec.fr's own frontend uses; paginates via `pagination.startIndex` up to `max_pages` (default 3). **Detail URL is best-effort, not verified**: apec.fr is a client-rendered SPA behind DataDome bot-protection — the constructed permalink returns HTTP 200 for any id, real or garbage, so a 200 is not proof it resolves to the right posting. Spot-check a generated URL in a browser before relying on this provider for anything downstream of the link itself. |
...
| Arbetsförmedlingen | API | Reads Sweden's public JobTech JobSearch API (`jobsearch.api.jobtechdev.se` — the open-data backend behind Platsbanken). Configure with `provider: arbetsformedlingen` and an `arbetsformedlingen:` block (`keywords`, falling back to `config/profile.yml` target_roles); paginates via `offset` up to `max_pages` (default 3). No numeric salary field — the API exposes only a taxonomy concept and free text, so `salary` is never emitted. |
...
| Diavgeia (Δι@ύγεια) | API | Greece's public-sector transparency register — every Greek public body must publish its hiring announcements here, so it is the canonical feed for research centres, universities, municipalities and ministries. Configure with `provider: diavgeia` and an `organizationUid` (find it at `https://diavgeia.gov.gr/opendata/decisions/{ADA}.json` → `organizationId`); `decisionTypeUids` defaults to `Γ.3.1` (ΠΡΟΚΗΡΥΞΗ ΠΛΗΡΩΣΗΣ ΘΕΣΕΩΝ). Reads `/luminapi/api/search`, the endpoint the Diavgeia UI itself calls — **not** `/opendata/search`, which currently answers 200 with every filter silently ignored. **Known ceiling:** the published subject is the funding project's title, never the role; the actual specialty lives only inside the attached PDF προκήρυξη, so title keyword filters cannot narrow these by role. Scope with `organizationUid` and triage by hand. |
...
| EURES | API | The EU-wide job mobility portal — one provider covering every EU/EEA national employment service. Configure with `provider: eures` and an `eures:` block (`keywords` falling back to `config/profile.yml` target_roles, optional `location_codes` ISO country filter, `publication_period`); paginates up to `max_pages` (default 3). POSTs to `europa.eu/eures/api/jv-searchengine/public/jv-search/search`, a community-documented (reverse-engineered, not official) endpoint — treat breaking changes as expected. `location` is the bare country code only (resolving NUTS codes to city names would cost a second network call per posting); no salary field is exposed. |
...
| freehire | API | Reads the public, no-auth `https://freehire.me/api/v1/jobs/search` API (~50 source ATS platforms normalized into one schema, multi-market via `countries`/`regions`/`remote` facets — verified live: `countries=dk&is_tech=tech` alone returns 2337 real Danish tech postings, zero Danish-specific code). Configure with `provider: freehire` and a `freehire:` block (`keywords` falling back to `config/profile.yml` target_roles, optional `countries`, `regions`, `remote`, `is_tech`, `posted_within_days`); paginates via `offset` up to `max_pages` (default 3). `FREEHIRE_API_URL` env var points it at a self-hosted instance (validated as http/https, never followed through a redirect). No salary field — `salary_period` is per-posting (year\|month\|day\|hour) with no normalized currency, and a wrong day/hour→annual guess silently drops well-paid roles with no visible error. |
