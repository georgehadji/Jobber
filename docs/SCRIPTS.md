...
| `npm run add` | `add-entry.mjs` | Dedup + insert a `/jobber add` entry into cv.md / article-digest.md |
...
Validates that the Jobber setup is internally consistent: `cv.md` exists and is not too short, `config/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `article-digest.md` freshness (warns if older than 30 days).
...
Checks whether a newer version of Jobber is available upstream. Outputs JSON to stdout:
...
```bash
JOBBER_DNS_LOOKUPS_PER_MIN=800 npm run scan:full   # raise the ceiling
JOBBER_DNS_LOOKUPS_PER_MIN=0 npm run scan:full     # no pacing (pre-#2229 behaviour)
JOBBER_NO_DNS_CACHE=1 npm run scan:full            # no DNS cache AND no pacing
```
