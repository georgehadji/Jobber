...
| `interview-prep/{company}-{role}.md` | Company-specific interview prep reports (written by `/jobber interview-prep`) |
...
| `data/company-intel/{slug}.md` | Your pasted employer-review notes, one file per company (third-party PII-adjacent — gitignored, never fetched or scraped, you paste it yourself). Read by `company-intel.mjs`, surfaced only as facts-and-quotes research context — never a source for CV/cover-letter/application-answer generation |
...
| `documents/{cv,linkedin,diplomas,references}/*` | Your existing career documents (CV, LinkedIn export, diplomas, reference letters), read by `node ingest-documents.mjs` during onboarding — never fetched or scraped, you place them yourself (except `documents/README.md` and each subdirectory's `.gitkeep`, which are system-owned scaffold delivered by updates). Same trust level as `cv.md`: a source you can point at, not a source of truth until its content is merged into the actual user-layer files with your confirmation |
