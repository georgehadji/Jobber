...
/**
 * origin ("scheme://host") -> { expiresAt, ...one of: }
 *   { result }  a path-INDEPENDENT verdict (fetch/parse failure — same for
 *               every path on this origin) — SEED-A: never cache the final
 *               allow/disallow decision here, it's path-specific.
 *   { groups }  the parsed robots.txt rule groups — isAllowed() is re-run
 *               per path against these on every cache hit.
 */
const cache = new Map();
...
export async function gate(url, { fetchText = defaultFetchText, agent = DEFAULT_AGENT, now = Date.now } = {}) {
  ...
  if (cached && cached.expiresAt > now()) {
    return cached.result || decide(cached.groups, agent, path);
  }
  ...
  cache.set(origin, { groups, expiresAt: now() + CACHE_TTL_MS });
  return decide(groups, agent, path);
}

/** Path-specific allow/disallow verdict from a parsed robots.txt rule set. */
function decide(groups, agent, path) {
  if (!isAllowed(groups, agent, path) || !isAllowed(groups, '*', path)) {
    return { allowed: false, reason: `DISALLOWED for ${agent} or *` };
  }
  return { allowed: true, reason: 'ALLOWED — robots.txt permits this path' };
}
