/**
 * lib/token-tracker.mjs — Token tracking and cost estimation for Jobber
 *
 * Model ids and prices are NOT declared here — they come from the single
 * source of truth in lib/llm-providers.mjs. RATES is re-exported for the
 * existing importers (tests, batch/aggregate-tokens.mjs, utils/ shim).
 */
import { RATES, providerSpec, pinnedModelFor } from './llm-providers.mjs';
...
export function estimateCost(model, usage, provider) {
  if (providerSpec(provider)?.free) return 0;
  if (provider === 'openrouter' && !pinnedModelFor('openrouter')) {
  ...
  if (!rate) {
    // Unknown model id: price it as the provider's declared default. Providers
    // with no default (openrouter rotates live) stay unpriced.
    // NOTE: the provider default, not defaultModelFor() — a user-pinned env
    // model that isn't in RATES must not be silently priced as itself.
    const fallbackModel = providerSpec(provider)?.defaultModel;
    rate = fallbackModel ? RATES[fallbackModel] : null;
    if (!rate) return null;
  }
