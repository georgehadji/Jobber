...
export const PROVIDERS = {
  ...
  openrouter: {
    ...
    // `deny` restricts OpenRouter to providers that do not retain prompts
    // (their `data_collection: "deny"` routing filter). Read via
    // openrouterProviderPrefs(); the hosted tier sets it unconditionally.
    dataPolicyEnv: 'OPENROUTER_DATA_COLLECTION',
  },
...
/**
 * Hosted-tier model routes over OpenRouter, one fallback chain per workload.
 * Decided 2026-09-08 from live OpenRouter list prices and the endpoint pool
 * behind each model (docs/HOSTED-APP-PLAN.md §5 has the table and the reasoning).
 *
 * Order matters: primary → a different vendor of the same class → a floor that
 * is boring and rarely down. OpenRouter walks the list on downtime, rate limit,
 * context overflow or moderation, and bills the model that answered.
 *
 *   chat      the assistant turn: short, frequent, must emit action envelopes
 *   ingest    CV → cv.md: "invent nothing", long PDFs, precision over prose
 *   evaluate  the A–G judgment: the product; the 12K static prefix caches at 0.1×
 *   write     CV tailoring, cover letters, form answers: needs a writer
 *
 * The CLI's `spend_tier` (SPEND_TIER_MODELS) is untouched — that routes the
 * user's own coding CLI; this routes the hosted tier's own key.
 */
export const HOSTED_ROUTES = {
  chat:     ['z-ai/glm-5.3-flash', 'deepseek/deepseek-v4-flash', 'google/gemini-3.8-flash'],
  ingest:   ['deepseek/deepseek-v4-flash', 'z-ai/glm-5.3-flash', 'google/gemini-3.8-flash'],
  evaluate: ['deepseek/deepseek-v4-pro', 'qwen/qwen3.7-plus', 'google/gemini-3.8-flash'],
  write:    ['deepseek/deepseek-v4-pro', 'moonshotai/kimi-k2.6', 'google/gemini-3.8-flash'],
};

/**
 * A workload's chain as the comma-separated spec the runners accept in
 * `--model` / OPENAI_MODEL. Unknown workloads get the chat chain.
 *
 * @param {string} workload One of the HOSTED_ROUTES keys.
 * @returns {string} e.g. "deepseek/deepseek-v4-pro,qwen/qwen3.7-plus,google/gemini-3.8-flash"
 */
export function hostedModelSpec(workload) {
  return (HOSTED_ROUTES[workload] ?? HOSTED_ROUTES.chat).join(',');
}

/** Hostname of the OpenRouter gateway; runners compare their endpoint host to it. */
export const OPENROUTER_HOST = 'openrouter.ai';

/**
 * OpenRouter `provider` routing preferences, or null when none are requested.
 * `require_parameters` rides along with `deny` so a provider that silently
 * drops `tools` or `response_format` is never picked.
 *
 * @returns {{ data_collection: 'deny', require_parameters: true } | null}
 */
export function openrouterProviderPrefs() {
  const policy = (process.env[PROVIDERS.openrouter.dataPolicyEnv] || '').trim().toLowerCase();
  return policy === 'deny' ? { data_collection: 'deny', require_parameters: true } : null;
}

/**
 * The model field(s) for a chat-completions request body.
 *
 * A comma-separated spec is a fallback chain. On OpenRouter it becomes the
 * `models` array (tried in order); every other host gets the primary alone,
 * because `models` is OpenRouter-specific and some gateways reject unknown
 * fields. Provider preferences are attached on OpenRouter only.
 *
 * @param {string} modelSpec "id" or "id1,id2,id3".
 * @param {string} host Endpoint hostname, e.g. "openrouter.ai".
 * @returns {{ model?: string, models?: string[], provider?: object }}
 */
export function routingFields(modelSpec, host) {
  const chain = String(modelSpec ?? '').split(',').map((s) => s.trim()).filter(Boolean);
  const onOpenRouter = host === OPENROUTER_HOST;
  const out = onOpenRouter && chain.length > 1 ? { models: chain } : { model: chain[0] ?? '' };
  if (onOpenRouter) {
    const prefs = openrouterProviderPrefs();
    if (prefs) out.provider = prefs;
  }
  return out;
}
...
export const RATES = {
  ...
  // Hosted-tier routes (HOSTED_ROUTES) — OpenRouter list prices, 2026-09-08.
  // cachedInput is the cache-read rate the model's vendor bills through OpenRouter.
  'z-ai/glm-5.3-flash':        { input: 0.075 / 1000000, output: 0.25 / 1000000, cachedInput: 0.015 / 1000000 },
  'deepseek/deepseek-v4-flash': { input: 0.089 / 1000000, output: 0.177 / 1000000, cachedInput: 0.018 / 1000000 },
  'deepseek/deepseek-v4-pro':   { input: 0.955 / 1000000, output: 1.911 / 1000000, cachedInput: 0.08 / 1000000 },
  'qwen/qwen3.7-plus':          { input: 0.32 / 1000000, output: 1.28 / 1000000, cachedInput: 0.064 / 1000000 },
  'moonshotai/kimi-k2.6':       { input: 0.95 / 1000000, output: 4.00 / 1000000, cachedInput: 0.16 / 1000000 },
  'google/gemini-3.8-flash':    { input: 0.75 / 1000000, output: 3.75 / 1000000, cachedInput: 0.075 / 1000000 },
...
if (invokedPath && import.meta.url === pathToFileURL(invokedPath).href) {
  const spendFlag = process.argv.indexOf('--spend-tier');
  const routeFlag = process.argv.indexOf('--hosted-route');
  if (routeFlag !== -1) {
    // web/src/app/api/run's hosted branch shells out here for the same reason
    // batch-runner.sh does below: it cannot import an ES module across the
    // web/ → root boundary, so it reads the chain through this shim instead
    // of restating HOSTED_ROUTES.
    process.stdout.write(hostedModelSpec(process.argv[routeFlag + 1]));
  } else if (spendFlag !== -1) {
    process.stdout.write(spendTierModel(process.argv[spendFlag + 1]));
  } else {
    process.stderr.write('usage: node lib/llm-providers.mjs --spend-tier <economy|standard|premium>\n');
    process.stderr.write('       node lib/llm-providers.mjs --hosted-route <chat|ingest|evaluate|write>\n');
    process.exit(2);
  }
}
